...
Uses the WinRing0 kernel driver for privileged I/O port access.
...
class WinRing0:
    ...
    Self-installs and manages the WinRing0 driver.
    ...
    def uninstall(self):
        """Completely uninstall the driver service and remove its files."""
        # 1. Close handle if open
        self.close()
        ...
        if sc_manager:
            ...
            if service:
                # Stop
                status_buf = ctypes.create_string_buffer(28)
                self._advapi32.ControlService(service, 0x01, status_buf)
                # Delete
                self._advapi32.DeleteService(service)
                self._advapi32.CloseServiceHandle(service)
            self._advapi32.CloseServiceHandle(sc_manager)

        # 3. Remove scheduled task if it exists
        try:
            subprocess.run(['schtasks', '/delete', '/tn', 'FanControlAutoRestore', '/f'],
